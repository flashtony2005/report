...
//!
//! 引第三方 crate 也行，但那会为了**测试用途**把 MSRV 抬上去（`image` / `png`
//! 会拖进 `moxcms` / `pxfm`，要 Rust 1.88 / 1.85），代价落在一个跑在
//! 客户机上的服务上，不划算。这里延续 `chartkit` 自研的路子：算法是死的、
//! 规范是公开的，**正确性有解码 oracle 兜底**（`scripts/verify-barcode.py`
