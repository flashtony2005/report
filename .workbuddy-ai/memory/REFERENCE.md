...
### §二十一.11 第 4 档第 1 条·补做：服务端 409 + `force`（2026-09-27）

§二十一.10 的 A 那道确认查的是**列表快照** → 挡不住「别的客户端之后新建的同名」。
当天补上服务端那道闸（详见 `架构体检-不足与改进方案.md` §9.6）。

**两道闸查的不是同一个事实**：UI 查快照，服务端查**文件系统**。互补，不是重复。

**契约**（`PUT /api/reports/save`）：

- 目标已存在 + 没带 `?force=1` → **409**，且**零副作用**（文件逐字节不变、不留 `.bak`）。
  只断言状态码不够 —— 「先覆盖了再返回 409」也能过，那比不报错更坏。
- `?force=1` 才穿透。**只认 `1` / `true` / `yes`**；`0` / `flase` / 空值**一律当没带** ——
  语义是「**证明**你知道要覆盖」，不是「提过这事」。
- **非法 id 仍是 400**（先校验再查存在性），别和 409 混。
- `force` 走**查询串**不走请求体：请求体是 `ReportDef` 本体、要**原样落盘**，
  塞进去得靠 `#[serde(skip)]` + TS 同步忽略。

**三个必须记住的点**：

1. `SaveError{Invalid, Conflict}` 必须**能区分** —— 否则 UI 只能靠**匹配文案**分辨
   「弹错误」还是「弹覆盖确认框」，文案一改（哪怕加个标点）就静默失效。
2. UI 里 **409 分支必须排在通用 `!res.ok` 之前**。落进通用分支 → 用户看到
   「报表 x 已存在」却**没有任何办法继续保存**，明明点一下「覆盖」就能存。
3. **`normalise_id` 只此一处**。`save` 会 trim → `" t8 "` 落盘成 `t8.json`；
   `save_new` 若拿**没 trim** 的串查存在性 → 判「不存在」→ 放行 → 覆盖掉 `t8.json`。
   **不报错、不警告，安静地毁数据。** 有用例 + 注入 S2 钉住。
4. 「**存我自己正在编辑的那份**」要带 `force`（`id === lastSavedId`）。
   不带的话第二次保存被 409 顶回来 → 用户看到「存自己刚存过的那份也要确认」
   = 把闸做成了绊脚石。

**验证**：

- 新探针 `scripts/verify-save-conflict.py` **6 组判据全绿**（无 force 被拒且逐字节不变 /
  有 force 穿透 / typo 不当授权 / 非法 id 是 400）。
- 新注入 `scripts/fault-inject-save-conflict.py` **6 条 / 7 道门禁 7/7**。
- `fault-inject-save-confirm.py` 8 条 **8/8**；UI 用例 6 → **8**。
- `check-all.sh` 闸 7 = **144 用例 / 12 文件**（全量 **383** / 44 文件）。

**S4 / S5 只能靠真机探针**：`Conflict` → 400 这种错在**函数层完全看不出来**
（`save_new` 老实返回了 `Conflict`，单测全绿），错的只是 handler 翻成哪个状态码 ——
而 UI 正是靠状态码决定弹错误还是弹确认框。

**踩的两个坑**：

1. **改契约要 grep 全部调用方**：`/api/reports/save` 除 UI 还有三个 ——
   `verify-atomic-save.py`（第 2 步故意覆盖 v1，要带 force）、`verify-report-paper.py`、
   `report-store-test.html`。不改就全被新契约打成 409，而症状看起来像「功能坏了」。
2. **改用例标题会让注入假报漏网**：注入脚本写死了「该红在哪条用例上」，
   把 `点「覆盖」→ 请求真的发出去` 改名成 `…带 force=1 重发` 后忘了同步 →
   注入 5 报「红了，但不是期望的那条」。方向是**安全**的（宁可多报），
   但看到那条时**先看标题漂没漂**，别急着怀疑判据。

**仍未做（诚实标注）**：`force` **不是乐观锁** ——
「我打开它 → 别人改了它 → 我保存」仍会覆盖掉别人的改动。
要防得用 `updatedAt` 做 `If-Match` 式比对。
另：探针第 6 步断言的「先校验 id 再查存在性」那个**顺序**，六条注入**没有一条能让它红**
（顺序反了 `bad/id` 照样被 `save` 拦成 400）→ 它是**代码阅读得出的结论，
不是被注入证明的**（已诚实写在 `fault-inject-save-conflict.py` 顶部）。
