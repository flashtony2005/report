...
- **报表页面设置**（纸张/方向/边距/页码/居中）：`PageConfig` 后 5 个 `Option` → HTML `@page`（只吐作者明写的项）+ xlsx `pageSetup`/`pageMargins`/`oddFooter`。页码只能服务端烤进 HTML。**B5 = JIS 182×257**（Excel 码 13）。**背景 / 水印没做**。细节 §十六。
