...
**⚠️ 但这三条闸 + 27 个 `.py` 脚本全靠人记得敲**：无 CI、无 `check-all`、**没有 shell 脚本调用过 `.py`**。
→ **「没有跑器的闸」比「红的闸」更坏**（绿的 → 虚假信心）。`mirror-check.py` 是唯一查 Rust↔TS 漂移的闸，
却**只对形状（24 组字段 + 纸张名）不对语义**，规则常量全无闸。全文 `架构体检-不足与改进方案.md`。
