...
**缺口**（全仓 grep 确认这几类调用一个都没有）：

| 项                | 现状                      | 影响                            |
| ---------------- | ----------------------- | ----------------------------- |
| 单元格边框             | 无 `set_border`           | 中国式报表几乎都有边框，导出后是「裸表」        |
| 表头样式              | **只有第一行**加粗 + 底色        | 多级表头的第二行起没有样式                 |
| 正文样式              | 仅合并格居中，其余不带任何 Format    | 无对齐 / 字体 / 行高                 |
| 行高                | 无 `set_row_height`       | 全靠自适应                         |
| 打印设置              | 无纸张 / 方向 / 缩放 / 重复表头行    | 导出的 xlsx **打印时表头不会跨页重复**      |
| 图片 / 条件格式         | 无                        | 二维码、logo、数据条都出不来              |
| 列宽                | `clamp(8, 40)`           | 长文本显示不全                       |

「重复表头行」那条对多页报表是硬伤，且实现成本很低（rust_xlsxwriter 有
`set_repeat_rows`）。要不要补取决于导出后是不是直接拿去打印。
