...
| 上游规则                                                                  | 位置                                    | 我们  |
| --------------------------------------------------------------------- | ------------------------------------- | --- |
| 向左扫到相邻格**不是**扩展格、但它自己有 `rowParent` → 跟随到那个父格（递归）                    | `getRowParent` L269-273               | ❌   |
| 向左扫不到 → 取**最左格**的 `rowParent`；向上扫不到 → 取**第一行**的 `colParent`          | L277-285 / L327-335                   | ❌   |
| 扩展格的展开范围内、且**不与它同行**的格子，缺省都以它为行父格（`addDefaultRowParents`）           | L647-664                              | ❌   |
| `CellPosition.NONE` 显式表示「无父格」（等价我们的 `A0`）；`checkLoop` 查父格链成环并抛 `ERR_…` | L249、L372                             | ⬜ 部分 |

第 1、3 条在日常模板里通常能被「向左扫最近的扩展格」覆盖到（扫描会跨过非扩展格继续找），
所以没做不至于出错；真正会暴露差异的是**非扩展格显式声明了父格**这类手写模板。
...
| `expandInplaceCount`                | 模板预留了空行时复用，不足才新增                      | ⬜ 未做（需模板预留行语义）                            |
