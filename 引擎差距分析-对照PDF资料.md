...
| 表达式**成环**（A1→B1→A1，或自引用 `A1 + 1` 写在 A1） | 不挂不崩，但**静默出空** | 告警 `X 的表达式存在循环引用（直接或间接引用了自己），该格按空值处理`（2026-09-19 补） |
...
2. ✅ **环检测**——结论修正后改修「父格查不到导致整格静默消失」的真实缺陷（见第 7 条）。
   **2026-09-19 补**：防无限递归的 `evaluating` 守卫一直在，但成环时**一声不吭**
   —— 用户看到的就是几个空格子。按本节「静默失败必须可见」的口径补了告警，
   每格只报一次。**菱形依赖（A1→B1、A1→C1、两者都→D1）不是环，不许告警**，
   有测试守着（告警误报比不报更糟，用户会直接忽略它）。
...
**已支持（2026-09-17 新增三项）**：**细边框**（所有写出的格子都带，含空格子）、
**打印时表头跨页重复**（`set_repeat_rows`）、**缩放到一页宽**（`set_print_fit_to_pages(1, 0)`，
纵向不限页数）、**多行表头整体加粗 + 底色**。
...
| 项          | 现状                        | 影响                                    |
| ---------- | ------------------------- | ------------------------------------- |
| 行高          | 按需写（见下节）                  | 只有真的要多于一行的行才写；其余交给 Excel 自适应          |
| 纸张大小 / 方向   | **刻意不设**                  | 用什么纸取决于现场打印机（A4 / 241 连续纸 / 标签纸），写死可能反而不对 |
| 图片 / 条件格式   | 无                         | 二维码、logo、数据条出不来                       |
| 列宽          | `clamp(8, 60)`            | 超 60 的文本靠换行兜住，不再被裁                    |

> **「无」只指 xlsx 导出这一条路**，别把它读成「项目里没有图片/图表」，那是三个不同的东西：
>
> | 能力 | 自由画布 | 服务端非线性报表（`CellTpl`） |
> | --- | --- | --- |
> | 图片 / 条码 / 二维码 | **有**（`PrintQrcode` / `data-binder` 里的 barcode） | 无 |
> | 图表 | **有**，自研 `openprint/src/core/chartkit`（bar/line/pie，纯函数出 SVG，**零第三方依赖**） | 无（`print-server/src` 里 grep `chart` 零命中） |
> | 导出 | 客户端 `export-engine/`（PDF/SVG/图片，`chart-svg-to-path.ts` 负责图表转矢量路径） | 服务端 xlsx / HTML |
>
> 所以「没有图表库」这个说法是错的 —— 是**没有引第三方图表库**，图表能力是自己写的，
> 且**只覆盖画布侧**。服务端渲染的报表要出图，得另做。

> 缩放到一页宽这一点值得说明：`set_print_fit_to_pages` 会把 `print_scale` 固定成 100，
> 所以它**只会缩小、不会放大** —— 窄表不会被撑大。高度给 0 = 纵向该几页就几页。
> 纸张与方向留给 Excel / 现场打印机，代码不猜。

#### 列宽上限为什么是 60（不是不设，也不是 40）

`column_widths()` = 该列最长文本的 `display_width`（全角算 2，**末尾 +2 内边距**），
再 `clamp(8, 60)`。三个数都是有来由的：

- **下限 8**：「备注」「编码」这类两字词只有 6 宽，不抬会挤成一条缝。
- **上限 40（旧值，已改）**：实测 23 个汉字的备注是 48 宽，被截掉一截 ——
  相邻列有内容时 Excel 是**裁掉**而不是溢出，所以是真的看不见了。
- **上限 60（现值）**：≈ 30 个汉字 / 60 个英文字符，覆盖常见的备注、地址、品名列。
- **为什么不干脆不设上限**：导出同时开了缩放到一页宽，**列越宽 → 缩放越狠 →
  打印出来整张表的字越小**。为了一段长备注牺牲整表可读性不划算。
  实测样例报表总宽 55、带长备注的表 64，都还在一页宽（约 11 列 ≈ 92）之内，
  不会触发额外缩小。

超过 60 的文本不再截断 —— 走下面的「换行 + 行高」。

#### 超长文本：换行 + 按需行高

超宽文本在 Excel 里是**被裁掉**而不是溢出（相邻列有内容时），所以列宽封顶之后
剩下的路只有换行。但光开 `set_text_wrap` 不行：Excel 只在「行高没被写死」时
才自动撑高，不同查看器行为还不一致。所以两件事一起做，而且**都是按需的**：

- `lines_needed(text, avail)`：每个 `\n` 段独立算 `ceil(宽 / 可用宽)` 再求和 ——
  硬换行是作者**要**断开的地方，不能跟自动换行混在一起取 max。
- `avail` 是**该格实际能用多少宽**：合并格要把跨过的列宽都加起来，
  否则一个横跨 3 列的长标题会被误判成要换行。
- 行高 = 行数 × **15pt**（`LINE_HEIGHT`）。15 是 Excel 自己的默认行高，
  用别的值会让「我们写死行高」与「Excel 自动行高」两种状态下行列对不齐。
- **只有 max(行数) > 1 的行才写行高**，其余行一个 `customHeight` 都不写 ——
  这样普通报表导出的行高跟以前**完全一样**，改动只落在真正需要的行上。
  同理，只有自己装不下的格子才套 wrap 格式，同行其它格子保持原样。

实测（`scripts/verify-xlsx-export.py --expect-height-rows N --expect-wrap yes/no`）：

| 场景 | 结果 |
| --- | --- |
| 23 字长备注（>60 宽） | 行高 30（2 行）、该格 wrapText |
| 含 `\n` 的文本 | 行高 30、`\n` 原样写进 sharedStrings |
| 横跨 3 列的长标题（合并后够宽） | **不换行**、不写行高 —— 合并宽度确实算进去了 |
| 内置样例 23 行 | 0 行自定义行高、文件里**没有** wrapText 样式 |
...
**怎么验**：这三件事**单测证明不了**（xlsx 是 zip，内容压缩过，Rust 侧只能断言
`to_xlsx` 不报错）。用 `scripts/verify-xlsx-export.py <文件> --expect-repeat N`
拆 zip 读 XML 校验：thin 边框存在、**每个格子都带格式**（默认样式 = 无边框）、
`_xlnm.Print_Titles` 存在且范围是 `$1:$N`、`pageSetUpPr fitToPage=1` 与
`fitToHeight="0"` 存在。

> 校验缩放时踩了个坑：`fitToWidth="1"` 是 XML 的**默认值**，写文件时会**被省掉**，
> 所以断言这个属性存在必然失败 —— 真正的开关是 `pageSetUpPr fitToPage="1"`。
> 第一版脚本在这里红了，是**读 XML 才发现**的。
...
- ODBC 配置块里直接写着「Rust 版客户端暂未实现 ODBC，配置可以保存，但暂时连不上」
  （`admin.html:322`）；
...
## 2026-09-19 实测盘点：还剩什么没做

这一节是**对着代码逐条 grep 出来的**，不是照着文档抄的。先说「别再当缺口」的，
再说真正没做的。

### 先排除：这些其实已经做了（我自己差点又记错）

| 项 | 证据 |
| --- | --- |
| 表达式函数集 | 官方 11 个全在，另加 `MAP` / `FILTER` / `REDUCE` / `FLATMAP` |
| `CellModel` 字段没死字段 | 19 个字段逐个统计引用，`engine.rs` 里都有真实读点（`expand_max_count` 1127、`keep_expand_empty` 1131、`expand_expr` 1113、`join_on` 754） |
| 分页 | `paginate()`（`mod.rs:319`），`rows_per_page` / `repeat_header_rows` / `repeat_footer_rows` 三个都生效，不是死配置 |
| 多 sheet 导出 | `xlsx.rs:37` 循环 `add_worksheet()` |
| 图片 / 条码 / 二维码 | 自由画布侧有（`PrintQrcode`、`data-binder` 里的 barcode） |
| **图表** | **自研 `openprint/src/core/chartkit`（bar/line/pie，纯函数出 SVG，零第三方依赖）** —— 上一版我写「没有图表库」是错的，已在上表订正 |

### 真正的缺口，按影响排序

**1. 格子样式 —— ✅ 2026-09-20 已补**（模型 + xlsx + 镜像 + 设计器面板，见下节）

`GridCell`（`model.rs:379`）一共只有 7 个字段：`text` / `pos` / `rowspan` /
`colspan` / `raw_number` / `num_format` / `formula`。**没有字体、字号、字色、
底色、边框、对齐、条件样式中的任何一个。**

设计器里看到的彩色是**语义高亮**（`grid-report.ts:661-666`：纵向扩展黄、
横向扩展绿、字段绑定蓝、表达式紫斜体，外加表头 / 选中 / 主格高亮），
标记的是「这格是什么角色」，不是「这格长什么样」，也不会导出。

导出到 xlsx 时样式全靠导出器写死的四样：表头加粗 + 底色、全体细边框、
按需换行行高、缩放到一页宽。**用户改不了任何一个。**

→ 要补就得给 `CellTpl` 加一层 `style`（前后端模型 + mirror-check + xlsx 三个
通道 + 设计器面板），是条完整链路，不是小改动。

**2. 分页不认分组 —— ✅ 2026-09-20 已补**（不许在合并格中间切页，见下节）

`paginate()` 是**渲染完之后**按固定行数切页，它看到的是拍平的网格，
不知道哪几行属于同一个地区。`mod.rs:313` 的注释把边界写明白了：
「只做按固定行数切页这一层，不引入润乾那套 9 类带区模型」。

后果：某地区的明细跨页时，第二页顶部只有重复的表头，**没有重复的地区名**
（`repeat_header_rows` 重复的只是模板前 N 行，补不出运行时展开出来的主格）。
要做「组内不跨页 / 每组另起一页」，得引入带区模型或至少加一个「行后分页」标记。

**3. 图表 / 图片进服务端报表 —— ✅ 2026-09-20 已补**（图片格 `CellImage`，见下节）

原状：画布侧有图，`print-server/src` 里 grep `chart` 零命中，服务端渲染的
非线性报表出不了 logo / 二维码 / 图表。

补法选了「客户端栅格化 + 服务端只嵌字节」这条路，理由是图表本来就是前端画的
（`openprint/src/core/chartkit` 纯函数出 SVG），在 Rust 里再实现一遍折线 / 柱状
等于维护两套画法。于是：

- 模板侧新增 `CellImage { from, src }`，`src` **只收 data URI**（不收文件路径 ——
  服务端按模板字符串读本地文件等于把模板变成任意文件读取原语，而模板可被分享）；
- 服务端把它落到 `GridCell.image`，xlsx 侧用 `rust_xlsxwriter` 的 `Image`
  嵌进 `xl/media/`，HTML 预览侧出 `<img>`；
- `from: value` 让「一列产品图」逐行取源（`field: photo` + `image.from: value`）。

**顺带纠正上一版的一个错误结论**：我先前说「插图要开 `image` feature、
会拉进 `rust_image` 一整棵依赖树」。查了 vendored 源码后发现是**错的** ——
`rust_xlsxwriter` 0.99 **没有** `image` feature，PNG/JPEG/GIF/BMP 的头解析
是它自己用纯 Rust 写的（`Image::new_from_buffer`）。所以这一刀没加任何新依赖。

仍**不**支持的：webp / svg（明确报错，不静默）；服务端自己画图表（不做）。
...
全仓 `params` 只出现在 `/api/data/rows` 的 SQL 绑定参数上（`db.rs:38`）；
没有「打开报表前弹查询条件 → 注入过滤 → 再展开」的概念。
润乾 / NopReport 都有参数表单，这属于**缺失的整个模块**，不是某个字段。

**7. CSV 导出 —— ✅ 2026-09-20 已补**（`POST /api/report/csv`，带 BOM / RFC 4180 转义）

**8. MySQL —— ✅ 2026-09-20 已改成明确报错**（不再静默降级成 sqlite）

`normalizeDbEngine('mysql')` 归一成 `sqlite`（有测试钉住：
`print-client-data.spec.ts:95`）。不过 UI 的下拉只给
`sqlite` / `postgres` / `odbc` 三项（`admin.html:407`），
所以只有手改配置文件才会踩到 —— 优先级最低，但真踩到会看到
「sqlite 文件不存在」这种误导性报错。

### 一句话结论

**引擎的内核（展开、层次坐标、表达式、主格、跨数据集、分页、告警）已经齐了，
缺的是「长相」和「外延」**：格子样式、分组感知的分页、图表进服务端、
票据打印、参数表单。

**2026-09-20 更新**：标 ✅ 的项（格子样式、分组分页、CSV、MySQL 报错、参数表单、
票据指令、**图片格**）**已经补完**，只剩一项真正的「外延」：**ODBC 引擎**。

**2026-09-20 再更新**：八项缺口里七项已补，唯一剩下的是 ODBC（`db.rs` 里
...
- `with_style` 是**叠加**在导出器基础格式上的：作者没设的项不动，
  表头加粗、全体细边框都保留。
- **刻意不给边框**：Univer 的 `bd` 实测完全不渲染，设了在设计器里看不见，
  那就是「设了没反应」的静默失败。宁可不给，也不给一个看不见的开关。
- **颜色只认 `#RRGGBB`，认不出来报 HTTP 500**（点名「格子 A3 的 style.color
  「red」不是 #RRGGBB」），不静默丢弃 —— 静默等于作者设了没变化。
- 真机拆包探针（`xl/styles.xml`）：红字 `FFFF0000` / 黄底 `FFFFF1B8` /
  字号 14 / 加粗 / 居中全部落地；未设样式的报表 fonts 仍是 2 个，**无回归**。

### 分页不切断合并格（组内不跨页）

**关键洞察：主格展开出来就是合并格** —— 一个地区跨几行，它的格 `rowspan`
就是几。所以「不许在合并格中间切页」**等于**「组内不跨页」，
不用引入润乾那套 9 类带区模型，前后端模型都不用动。

- 纯函数 `merge_blocked_boundaries()` 标出哪些行边界不能切。
- 落点被合并格跨过时**往两边找最近的安全边界**（先退后进），
  页大小贴着 `rows_per_page` 走，不会一个劲儿往后胀。
- 剩下的够放一页时不再切（否则会为了「凑满」硬拆出一张小页）。
- 代价：页大小不再严格等于 `rows_per_page`；一个组超过一页时那页必然超
  —— **宁超不切**。
- 样例实测：华东 8 行 / 华南 6 行 / 华北 6 行，切页按组对齐 **8 / 6 / 7**
  （旧行为是 10 / 10 / 1，会把地区腰斩）。

### CSV 导出

`POST /api/report/csv` + `GET /api/report/sample.csv`。
RFC 4180 转义 + UTF-8 BOM（缺了 Excel 开中文乱码）+ CRLF。
写 `text` 不写 `formula`（CSV 里 `=SUM(...)` 只是文本，语义完全不同）。

**CSV 注入（`=cmd|`）刻意不改写**：加 `'` 前缀能防住，但会把 `+86` 开头的
真实手机号也改掉，属于拿正确性换安全。本服务数据来自自己配置的库；
真要导不可信数据应在消费侧处理。这个取舍写进了 `csv.rs` 顶部注释。

### MySQL 不再静默降级成 sqlite

`norm_engine` 对未知引擎一律回落 sqlite，而 `ServerConfig::load()`
**并不调 `validate()`**（只有保存 / 试连才调），所以手改配置写
`engine: "mysql"` 是真能踩到的 —— 报「sqlite 文件不存在」，
看着像路径写错，其实是引擎根本不支持。

新增 `unsupported_engine_name()`，在建连接与试连处拦截并点名引擎。

### 报表参数 / 查询表单

原来只有 `RunRequest.params`（数据集名 → 位置参数数组）那条底层通道：
调用方得自己知道 SQL 里第几个 `?` 是什么，前端没法据此画表单。
润乾 / NopReport 都有的「执行前弹查询条件」这一层之前完全没有。

- `ReportDef.params: Vec<ReportParam>`（name / label / kind / default / required /
  options）—— UI 可据此**自动生成查询表单**（`kind=enum` 时 `options` 就是下拉项）。
- 运行请求新增 `values`（参数名 → 值）。
- 绑定：数据源 `params` 里**整体等于** `"$name"` 的字符串换成值。
  只认整体等于，不认「包含 `$`」—— 宽松匹配会把作者真想写进 SQL 的 `$` 也换掉。

三种情况一律**报错**（静默的后果都是「筛选没生效，作者以为生效了」）：
传了没声明的参数（typo）、必填但没值、引用了解析不出值的参数
（拿 Null 顶会让 SQL 筛「字段 IS NULL」，看着像筛过了其实筛的是另一回事）。
