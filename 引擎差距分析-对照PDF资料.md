...
**2026-09-16 晚：已实现 `MAP` / `FILTER` / `REDUCE` + Lambda**（用户选定为下一步）。

- 语法：`x => x * 2`（单参数）/ `(acc, x) => acc + x`（多参数）。
- `MAP(set, fn)` → **值列表**（新增 `Val::List`）；`FILTER(set, fn)` → 输入是格集时
  **保留格实例**，所以 `FILTER(C1[A1:+0], x => x > 100).sum()` 仍走格集聚合口径；
  `REDUCE(set, fn, 初值)` → 标量。
- 聚合函数（`SUM` / `AVG` / …）现在也吃 `List`，于是 `SUM(MAP(C1, x => x * 2))` 可用。
- 与既有 `Filter`（`{}`）**不是一回事**：`{}` 是润乾式「候选格上下文」过滤，
  条件里裸 `B2` 指候选格；`FILTER()` 的 lambda 参数拿到的是**值**。两者并存。
