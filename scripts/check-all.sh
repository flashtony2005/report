...
# ## 不含什么
#
# 19 个 `fault-inject-*.py` 与 15 个 `verify-*.py` **不在这里**：前者要改源码、
...
#  `verify-save-conflict.py` 核成 19 / 15。
...
会跑的闸（按执行顺序）：
  ...
  7. bash    scripts/ts-test-designer.sh grid-report-  设计器 UI 单测（jsdom，串行，12 文件 / 144 用例，~145s）
--fast 只跑 1、2。

第 7 道**带过滤参数**（`grid-report-`，12 文件 / 144 用例），不是整套 designer-react：
...
printf '\n（不含 19 个 fault-inject / 15 个 verify —— 要改源码 / 起服务，按需单独跑：`ls scripts/fault-inject-*.py`）\n'
